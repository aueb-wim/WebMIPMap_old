...
function loadSchemaTrees(taskName, JSONTreesData){
    ...
    //on window resize re-draw connections
...
function setMaxHeight(){
    //minimum height
    var maxHeight = 300;
    $('#schemaTabs-'+scenarioCounter+' .maindivchild').each(function() { 
...
function addViewTransformationsTab(scenarioCnt) {
    $.post("MappingServlet", {action:"show_mapping_task_info", scenarioNo: scenarioCnt}, function(responseText){ 
...
function addViewSqlTab(scenarioCnt) {
       $.post("MappingServlet", {action:"sql_output", scenarioNo: scenarioCnt}, function(responseText){ 
        var obj = $.parseJSON(responseText); 
        if(obj.hasOwnProperty("exception")){
            alert(obj.exception);                    
        } 
...
function addViewXQueryTab(scenarioCnt) {
    $.post("MappingServlet", {action:"xquery_output", scenarioNo: scenarioCnt}, function(responseText){ 
        var obj = $.parseJSON(responseText); 
        if(obj.hasOwnProperty("exception")){
            alert(obj.exception);                    
        } 
...
function saveTask(saveName){
    $.post("MappingServlet", {action:"save_mapping_task", saveName: saveName, scenarioNo : currentScenario}, function(responseText){ 
        var obj = $.parseJSON(responseText); 
        if(obj.hasOwnProperty("exception")){
            alert(obj.exception);                    
        } 
       ...
       buttons: {
        "OK": function(){ 
            ...
            if($.inArray(deleteName, openedTasks) !== -1){
                ...
                if (r === true) {
                    $.post("MappingServlet", {action:"delete_mapping_task", deleteName: deleteName}, function(responseText){
                        var obj = $.parseJSON(responseText); 
                        if(obj.hasOwnProperty("exception")){
                            alert(obj.exception);                    
                        }
...
function createSelectionConditionPopup(item_id){
    ...
    var dialog = $( "#selection-condition" ).dialog({
      ...
      modal: true,
       buttons: {
        "Delete": function(){
            $.post("MappingServlet", {action:"edit_selection_condition", specificAction:"delete", path:pathTillNode, expression:"", scenarioNo: scenarioNo}, function(responseText){
                var obj = $.parseJSON(responseText); 
                if(obj.hasOwnProperty("exception")){
                    alert(obj.exception);                    
                } 
        ...
        "OK": function(){                
            ...
            var valid = (expression!=="");
               if (valid){
                   $.post("MappingServlet", {action:"edit_selection_condition", specificAction:"update", path:pathTillNode, expression:expression, scenarioNo: scenarioNo}, function(responseText){              
                    var obj = $.parseJSON(responseText); 
                    if(obj.hasOwnProperty("exception")){
                        alert(obj.exception);                    
                    } 
                       ...
                       var image_tag ="<i class='jstree-icon jstree-themeicon jstree-themeicon-custom' role='presentation' style='background-image: url(\"images/selection_condition_set.png\"); background-position: center center; background-size: auto auto;'></i>";
                        ...
                        if (expression.length > 25)
                            $("#"+item_id).html(image_tag+$("#"+item_id).data('original_text')+" ["+expression.substring(0,25)+"...]");
                        else
    ...
    var items = {
        selectItem: {
            ...
            action: function () {
                ...
                $.post("MappingServlet", {action:"select_mapping_task", scenarioNo:scenarioNo}, function(responseText){
                    var obj = $.parseJSON(responseText); 
                    if(obj.hasOwnProperty("exception")){
                        alert(obj.exception);                    
                    } 
...
function makeTrees (sourceTreeArray, targetTreeArray, sourceId, targetId, newplumb, JSONData){
  ...
  newplumb.bind('connection',function(info){
        ...
        if(!(con.getParameter("toFunction"))){              
            ...
            if(lastAction!=="open"){
                $.post("MappingServlet", {action:"established_connection", sourcePathArray:sourcePathArray, targetPath:targetPath, 
                    sourceValue: sourceValue, expression: expression, scenarioNo: idNo}, function(responseText){
                var obj = $.parseJSON(responseText);
                if(obj.hasOwnProperty("exception")){
                    alert(obj.exception);                    
                } 
    ...
    //send join condition data to the server
        ...
        con.setParameter("scenarioNo",idNo);
        $.post("MappingServlet", {action:"new_join_condition", sourcePath:sourcePath, targetPath:targetPath, scenarioNo: idNo, isSource: isSource}, function(responseText){
                var obj = $.parseJSON(responseText); 
                if(obj.hasOwnProperty("exception")){
                    alert(obj.exception);                    
                } 
  ...
  newplumb.bind('connectionDetached',function(info){
    ...
    if(con.getParameter("join_condition")){
        $.post("MappingServlet", {action:"detached_join", sourcePath : con.getParameter("sourcePath"), targetPath: con.getParameter("targetPath"), 
            scenarioNo: idNo, isSource: con.getParameter("isSource")}, function(responseText){
                var obj = $.parseJSON(responseText); 
                if(obj.hasOwnProperty("exception")){
                    alert(obj.exception);                    
                } 
    ...
    //do not delete the existing connection from the server
      ...
      ||(con.getParameter("already_deleted"))){
        ...
        $.post("MappingServlet", {action: "detached_connection",sourcePath: con.getParameter("sourcePath"), targetPath: con.getParameter("targetPath"), scenarioNo: idNo}, function(responseText){
                var obj = $.parseJSON(responseText); 
                if(obj.hasOwnProperty("exception")){
                    alert(obj.exception);                    
                } 
   ...
   tree1. bind("ready.jstree", function(){
    ...
    makeSelectionConditionNodes(treeSource, JSONData, loadExistingSelectionConditions);
   }); 
...
function updateConstantConnection(id, newplumb, newValue){   
    ...
    if(existing_conns.length  !== 0){
        for (var i=0; i< existing_conns.length; i++){            
            $.post("MappingServlet", {action:"update_connection", sourcePath : existing_conns[i].getParameter("sourcePath"), targetPath: existing_conns[i].getParameter("targetPath"), 
                scenarioNo: existing_conns[i].getParameter("scenarioNo"), sourceValue:newValue, sourcePathArray: null, expression:newValue}, function(responseText){
                    var obj = $.parseJSON(responseText); 
                    if(obj.hasOwnProperty("exception")){
                        alert(obj.exception);                    
                    }
...
function updateFunctionConnection(id, newplumb, newValue){   
    ...
    if(existing_conns.length  !== 0){
        for (var i=0; i< existing_conns.length; i++){
            /*var sourcePathArray = new Array();
            var existing_conns_to = newplumb.getConnections({ target: $('#'+id).find('img').attr('id') });
                for (var j=0; j< existing_conns_to.length; j++){
                sourcePathArray.push(existing_conns_to[j]);
            }*/
            
            $.post("MappingServlet", {action:"update_connection", sourcePath : existing_conns[i].getParameter("sourcePath"), targetPath: existing_conns[i].getParameter("targetPath"), 
                scenarioNo: existing_conns[i].getParameter("scenarioNo"), sourceValue:null, sourcePathArray: existing_conns[i].getParameter("sourcePathArray"), expression:newValue}, function(responseText){
                    var obj = $.parseJSON(responseText); 
                    if(obj.hasOwnProperty("exception")){
                        alert(obj.exception);                    
                    }
...
function makeSelectionConditionNodes(tree, JSONData, callback){
    ...
    if(JSONData.selConditions.length > 0){
      callback(JSONData.selConditions);//,newplumb);
    }
}

//function that loads existing selection conditions
function loadExistingSelectionConditions(selConditions){
    var image_tag ="<i class='jstree-icon jstree-themeicon jstree-themeicon-custom' role='presentation' style='background-image: url(\"images/selection_condition_set.png\"); background-position: center center; background-size: auto auto;'></i>";
    for (var i =0; i < selConditions.length; i++){
        ...
        $("#"+node_id).data('selection_condition_text', expression);
        if (expression.length > 25)
            $("#"+node_id).html(image_tag+$("#"+node_id).data('original_text')+" ["+expression.substring(0,25)+"...]");
        else
            $("#"+node_id).html(image_tag+$("#"+node_id).data('original_text')+" ["+expression.substring+"]");
    }
}

//function that removes a mapping task and also deletes the corresponding tabs
function removeMappingTask(scenarioNo){
    $.post("MappingServlet", {action:"remove_mapping_task", scenarioNo:scenarioNo}, function(responseText){ 
        var obj = $.parseJSON(responseText); 
        if(obj.hasOwnProperty("exception")){
            alert(obj.exception);                    
        }
        ...
        switch($(this).attr("data-action")) {
            ...
            case "mandatory":                     
                ...
                $.post("MappingServlet", {action:"join_condition_options", sourcePath:con.getParameter("sourcePath"), targetPath:con.getParameter("targetPath"), 
                    scenarioNo: con.getParameter("scenarioNo"), isSource: con.getParameter("isSource"), changedOption:"mandatory"}, function(responseText){
                    var obj = $.parseJSON(responseText); 
                    if(obj.hasOwnProperty("exception")){
                        alert(obj.exception);                    
                    } 
            ...
            case "fk":
                ...
                $.post("MappingServlet", {action:"join_condition_options", sourcePath:con.getParameter("sourcePath"), targetPath:con.getParameter("targetPath"), 
                    scenarioNo: con.getParameter("scenarioNo"), isSource: con.getParameter("isSource"), changedOption:"fk"}, function(responseText){
                    var obj = $.parseJSON(responseText); 
                    if(obj.hasOwnProperty("exception")){
                        alert(obj.exception);                    
                    } 
...
function createContextMenu5(newplumb,idNo,event){
    ...
    $( "#contextmenu"+idNo+" li").bind( "click", function() {
        ...
        if (r===true){
            ...
            $.post("MappingServlet", {action: "delete_all_connections", sourcePathArray: connectionsToDeleteSource, targetPathArray: connectionsToDeleteTarget, 
                scenarioNo: idNo}, function(responseText){
                var obj = $.parseJSON(responseText); 
                if(obj.hasOwnProperty("exception")){
                    alert(obj.exception);                    
                } 
...
function createContextMenu6(nodeId,event){
...
function createContextMenu7(newplumb, nodeId ,event){
     $("#maindiv").append("<ul id='contextmenu"+nodeId+"' class='custom-menu'>\
                              <li class='makeSourceConn'>Make Source For Connection</li>\
                              <li class='makeTargetConn'>Make Target For Connection</li>\
                           </ul>");
    var offset = $("#maindiv").offset();
    var relativeX = (event.pageX - offset.left);
    var relativeY = (event.pageY - offset.top);
    $(".custom-menu").css("left", relativeX + "px");
    $(".custom-menu").css("top", relativeY + "px"); 
    $( "#contextmenu"+nodeId+" li.makeSourceConn").bind( "click", function() {
        $(".custom-menu").remove();
    ...
    $( "#contextmenu"+nodeId+" li.makeTargetConn").bind( "click", function() {
        $(".custom-menu").remove();
            ...
            if($('#'+list_id+' option[value="'+option_value+'"]').length === 0){
                ...
                if (file) {
                    ...
                    reader.onload = function(e) {
                        ...
                        $.post(servlet, {buttonPressed:"add", fileName: file.name, firstLine: allLines[0], processData: false, contentType: false}, function(responseText){
                            var obj = $.parseJSON(responseText); 
                            if(obj.hasOwnProperty("exception")){
                                alert(obj.exception);                    
                            }                 
...
function createNewMapTaskDialog(type){
    ...
    out += '</select>\
         ...
         <div id="relational'+type+'" class="inputMenu'+type+'">\
            ...
            <select required id="driver'+type+'" name="driver'+type+'"  onchange="setUri($(this))">\
                   <option value="" class="hidden_text"></option>\
...
$(document).ready(function(){
    ...
    $( "#export" ).click(function() {
        if (currentScenario!==0){
            window.location = 'MappingServlet?action=export_tgds';
            /*var obj = $.parseJSON(responseText); 
                if(obj.hasOwnProperty("exception")){
                    alert(obj.exception);                    
            } */            
